...
def strategy_macd(df):
    ...
    if m.iloc[-1]>s.iloc[-1] and h.iloc[-1]>h.iloc[-2]: return "BUY",  f"MACD bullish crossover"
    if m.iloc[-1]<s.iloc[-1] and h.iloc[-1]<h.iloc[-2]: return "SELL", f"MACD bearish crossover"
...
# ════════════════════════════════════════════════════════════
# PRICE MOVEMENT CHART (why take this trade)
# ════════════════════════════════════════════════════════════
def show_price_chart(symbol, pair_name, signal, entry, sl, tp1, tp2):
    ...
    # EMAs
    fig.add_trace(go.Scatter(x=dates,y=ema20, name="EMA 20",
        line=dict(color="#ffd700",width=1,dash="dot")))
    fig.add_trace(go.Scatter(x=dates,y=ema50, name="EMA 50",
        line=dict(color="#ff7f50",width=1,dash="dot")))
    fig.add_trace(go.Scatter(x=dates,y=ema200,name="EMA 200",
        line=dict(color="#da70d6",width=1,dash="dash")))

    # Support & Resistance lines
    fig.add_hline(y=resistance, line_color="#f85149", line_dash="dash",
                  annotation_text=f"Resistance {round(resistance,4)}",
                  annotation_position="right")
    fig.add_hline(y=support, line_color="#3fb950", line_dash="dash",
                  annotation_text=f"Support {round(support,4)}",
                  annotation_position="right")
    ...
    with col2:
        ...
        • {zone_note}<br>
        • {"✅ Trend and signal AGREE — higher probability" if ("BUY" in signal and trend_dir=="uptrend") or ("SELL" in signal and trend_dir=="downtrend") else "⚠️ Trading against the trend — lower probability, reduce size"}
        </div>""", unsafe_allow_html=True)
...
def build_ai_strategy(user_input, market_context):
    try:
        ...
        if not api_key:
            return "⚠️ No API key found. Add ANTHROPIC_API_KEY to your Streamlit secrets (see Settings page)."
...
Build a complete, specific trading strategy with these sections:
...
8. ❌ What to avoid / when NOT to trade
...
Be specific, practical and actionable."""
        ...
        if r.status_code==200:
            return r.json()["content"][0]["text"]
        return f"API error {r.status_code} — check your API key in Settings."
    except Exception as e:
        return f"Error: {e}"
...
# ════════════════════════════════════════════════════════════
    ...
    if entry:
        ...
        col1,col2=st.columns(2)
        if col1.button("🔔 Send Telegram Alert"):
...
To use AI Strategy Builder:
1. Go to [console.anthropic.com](https://console.anthropic.com) → create a free account → get your API key
2. In your Streamlit app folder, create a file `.streamlit/secrets.toml`
3. Add this line: `ANTHROPIC_API_KEY = "sk-ant-your-key-here"`
4. Restart the app
        """)

    st.info("Describe what you want and the AI builds a complete custom strategy for you.")
    ...
    with col2:
        ...
        custom_note=st.text_area("Additional requirements",
                                 placeholder="e.g. I only want breakouts, or I prefer trend following...")

    if st.button("🚀 Build My Strategy"):
        context = f"Style:{style} | Risk:{risk_level} | Pairs:{','.join(fav_pairs)} | Session:{session} | Level:{experience} | Notes:{custom_note}"
        user_input = f"Build me a {style.lower()} strategy for {', '.join(fav_pairs)} during {session} session. I am {experience.lower()} with {risk_level.lower()} risk. {custom_note}"
        with st.spinner("🤖 Building your strategy..."):
            strategy=build_ai_strategy(user_input,context)
            st.session_state.ai_strategy=strategy

    if st.session_state.ai_strategy:
        st.divider()
        st.subheader("📋 Your Custom Strategy")
        st.markdown(f"<div class='strategy-card'>{st.session_state.ai_strategy.replace(chr(10),'<br>')}</div>",
                    unsafe_allow_html=True)
        if st.button("💾 Save to Journal"):
            st.session_state.trade_journal.append({
                "Date":str(datetime.date.today()),"Asset":"Custom Strategy",
                "Signal":"AI BUILT","Entry":0,"SL":0,"TP1":0,
                "Confidence":0,"Result":"Strategy",
                "Notes":st.session_state.ai_strategy[:300]
            })
            st.success("✅ Saved!")
...
**Step 3 — Paste below and test!**
    ...
    if st.button("💾 Save"):
        st.session_state.telegram_token=token; st.session_state.telegram_chat_id=chat_id
        st.success("✅ Saved!")
...
# ════════════════════════════════════════════════════════════
    ...
    st.subheader("🤖 Anthropic API Key (for AI features)")
    st.markdown("""
    To enable AI Strategy Builder and AI News Analysis:
    1. Go to [console.anthropic.com](https://console.anthropic.com) → sign up free
    2. Create an API key
    3. Create file `.streamlit/secrets.toml` in your app folder
    4. Add: `ANTHROPIC_API_KEY = "sk-ant-xxxxxxxx"`
    5. Restart the app — AI features will work automatically
    """)
